...
namespace oop_assignment
{
    public partial class ManageMenu : Form
    {
        ...
        private void chkAvailable_CheckedChanged(object sender, EventArgs e)
        {
            ...
            bool available = chkAvailable.Checked;
            string availabilityText = available ? "Yes" : "No";  // Convert bool to "Yes"/"No"
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                cmd.Parameters.AddWithValue("@name", itemName);
                cmd.Parameters.AddWithValue("@price", price);
                cmd.Parameters.AddWithValue("@availability", availabilityText);  // Use string value
        ...
        private void LoadMenuItems()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                // Add a new column to show "Yes"/"No"
                dt.Columns.Add("AvailabilityText", typeof(string));

                foreach (DataRow row in dt.Rows)
                {
                    string availabilityStr = row["availability"].ToString();
                    bool isAvailable = availabilityStr.Equals("Yes", StringComparison.OrdinalIgnoreCase) || availabilityStr == "1";
                    row["AvailabilityText"] = isAvailable ? "Yes" : "No";
                }

                dataGridView1.DataSource = dt;

                // Optionally hide raw availability column
